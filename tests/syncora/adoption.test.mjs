...
test("one adopt command previews, seals, migrates, verifies, and retires a reviewed graph", async () => {
  ...
  try {
    ...
    assert.equal(
      preview.previewSummary.title,
      "Syncora adoption preview",
    );
    assert.equal(preview.previewSummary.sourceNotes.total, 2);
    assert.equal(preview.previewSummary.sourceNotes.reviewed, 2);
    assert.equal(preview.previewSummary.targetNotes, 3);
    assert.ok(preview.previewSummary.representativePaths.length <= 8);
    assert.equal(preview.previewSummary.fullDetails.optional, true);
    assert.equal(preview.previewSummary.canonicalMarkdownChanged, false);
...
test("adopt resumes after an unmarked predecessor is explicitly reviewed", async () => {
  ...
  try {
    ...
    assert.match(patchedAgents, /syncora-agent-hook:begin v12/);
  } finally {
    await rm(fixture.workspace, { recursive: true, force: true });
  }
});
...
test("adopt refuses residual custom activation outside an exact predecessor block", async () => {
  ...
  try {
    ...
    assert.match(patched, /syncora-agent-hook:begin v12/);
  } finally {
    await rm(fixture.workspace, { recursive: true, force: true });
  }
});
...
test("legacy adoption stages, shadow-tests, atomically cuts over, verifies, retires, and rolls back", async () => {
  ...
  try {
    ...
    assert.equal(agents.includes("syncora-agent-hook:begin v12"), true);
...
test("cutover without an exact predecessor marker requires a durable review attestation", async () => {
  ...
  try {
    ...
    assert.match(agents, /syncora-agent-hook:begin v12/);
